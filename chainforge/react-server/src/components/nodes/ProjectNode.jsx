...
function ProjectNode({ data = { projects: [], tasks: {} }, id }) {
  ...
  const inputHandles = useMemo(() => {
    return Object.entries(PROJECT_ATTRIBUTES.inputs).map(([key, config]) => (
      <div key={`input-${key}`} className="attribute-row">
        <Handle
          type="target"
        ...
        />
        <Text size="sm" className="attribute-name">
          {key}
  ...
  const outputHandles = useMemo(() => {
    ...
    return Object.entries(PROJECT_ATTRIBUTES.outputs).map(([key, config]) => (
      <div key={`output-${key}`} className="attribute-row">
        <Text size="sm" className="attribute-name" title={config.description}>
          {key}
        </Text>
        <Text size="sm" className="attribute-value">
          {selectedProject?.[key] || ""}
        </Text>
        <Handle
          type="source"
  ...
  // Add handleOnConnect callback
  const handleOnConnect = useCallback(() => {
    if (selectedProject) {
      // Map all available project attributes
      const projectAttributes = Object.keys(PROJECT_ATTRIBUTES.outputs).reduce(
        (acc, key) => {
          acc[key] = selectedProject[key] || "";
          return acc;
        },
        {},
      );
  ...
  }, [id, selectedProject, setDataPropsForNode, pingOutputNodes, mode]);
